...
class BaseModel(models.Model):
    ...
    def __Page(self, model, params):
        ...
        if sort != None or __sort__ != None:
            ...
            if order=='desc':
                # 传参判断，需要逆序
                if start_time and end_time and between_str:
                    datas = eval(
                        ''' model.objects.filter(**params).filter({}__range= [start_time, end_time]).order_by("-{}").all()'''.format(
                            between_str, sort))
                else:
            ...
            if start_time and end_time and between_str:
                datas = eval(
                    '''model.objects.filter(**params).filter({}__range= [start_time, end_time]).all()'''.format(
                        between_str, sort))
            else:
                datas = model.objects.filter(
                    **params
